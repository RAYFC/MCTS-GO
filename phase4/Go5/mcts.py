...
from gtp_connection import get_dict
PASS = "Pass"
...
class TreeNode(object):
    ...
    def expand(self, board, color,in_tree_knowledge):
        ...
        if in_tree_knowledge:
            p_dict = get_dict(board)
            self._children[PASS]._n_visits = p_dict[PASS][1]
            self._children[PASS]._black_wins = p_dict[PASS][0]
            for move in moves:
                if move not in self._children:
                    if board.check_legal(move, color) and not board.is_eye(move, color):
                        ...
                        self._children[move]._n_visits=p_dict[move][1]
                        self._children[move]._black_wins = p_dict[move][0]
        else:
            for move in moves:
                if move not in self._children:
                    if board.check_legal(move, color) and not board.is_eye(move, color):
...
class MCTS(object):
    ...
    def get_move(self,
        ...
        for n in range(num_simulation):
            ...
            self._playout(board_copy, toplay,self.in_tree_knowledge)
