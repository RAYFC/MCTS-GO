...
def split_list (big_list):
   return [big_list[i:i+4] for i in range(0, len(big_list), 4)]
class GtpConnection():
    ...
    def prior_knowledge_cmd(self,args):
        ...
        results = sorted(sorted(results,key=lambda x: x[0]), key=lambda x: x[3],reverse=True)
        response=''
        for i in results:
            response=response+i[0]+' '+str(i[1])+' '+str(i[2])+' '
        self.respond(response)

    def sort_function(self,moves,sims,winrate,wins):
        length=len(moves)
        results=[]
        winrate1,moves1,sims1,wins1=zip(*sorted(zip(winrate,moves,sims,wins)))
        for i in range(length):
            results.append(moves1[i])
            results.append(wins1[i])
            results.append(sims1[i])
            results.append(winrate1[i])
        return results
    def get_the_prob(self,move):
        ...
        return Feature.compute_move_gamma(Features_weight, all_board_features[move])
...
def sort_function(moves,sims,winrate,wins):
    length=len(moves)
    results=[]
    winrate1,moves1,sims1,wins1=zip(*sorted(zip(winrate,moves,sims,wins)))
    for i in range(length):
