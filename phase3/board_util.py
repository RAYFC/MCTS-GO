...
class GoBoardUtil(object):
    ...
    def generate_all_policy_moves(board,pattern,check_selfatari):
        ...
        if board._liberty(last_move,color)==1:
            points = []
            if GoBoardUtil.generate_atari_capture_moves(board, last_move,board.current_player) is not None:
                points.append(GoBoardUtil.generate_atari_capture_moves(board, last_move,board.current_player))
                points= GoBoardUtil.filter_moves(board, points, check_selfatari)
            if points !=[]:
                return points, "AtariCapture"
        if GoBoardUtil.atari_defense(board,board.current_player,last_move)[1]:
            points=(GoBoardUtil.generate_atari_defense_moves(board,board.current_player,GoBoardUtil.atari_defense(board,board.current_player,last_move)[0]))
            points = GoBoardUtil.filter_moves(board, points, check_selfatari)
            if points !=[]:
                return points, "AtariDefense"
        if pattern:
    ...
    @staticmethod
    def generate_atari_defense_moves(board,color,points_in_atari):
        empty_points = board.get_empty_points()
        return_points=[]
        for point_in_atari in points_in_atari:
            for point in empty_points:
                newboard = board.copy()
                newboard.move(point, color)
                if newboard._liberty(point_in_atari,color)>1:
                    return_points.append(point)
        return return_points
    @staticmethod
    def atari_defense(board,color,last_move):
        points=[]
        exist_points=[]
        neighbors = board._neighbors(last_move)
        for n in neighbors:
            if board.get_color(n)==color:
                exist_points.append(n)
        for point in exist_points:
            if board._liberty(point,color)==1:
                points.append(point)
                return points,True
        return None,False
    @staticmethod
    def generate_atari_capture_moves(board,last_move,color):
        empty_points = board.get_empty_points()
        for point in empty_points:
            newboard = board.copy()
            newboard.move(point,color)
            if newboard.board[last_move]==0:
                return point
        return None
    @staticmethod
